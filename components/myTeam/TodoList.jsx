...
const TodoList = ({ todolistId, title, creator, resTodo, resDoing, resDone }) => {
    ...
    const clickDeleteTodolist = (e) => {
        ...
        axios
            .delete(process.env.NEXT_PUBLIC_LOCALURL_BACK + '/todolist/delete', { params: data })
            .then((res) => {
                const reqData = {
                    userId: userId,
                    teamName: roomName,
    ...
    const onDropTodoSection = (e) => {
        ...
        axios
            .get(process.env.NEXT_PUBLIC_LOCALURL_BACK + '/todolist/change/tddType', {
                params: data,
            })
            .then((res) => {})
    ...
    return (
        <div className={'my-4'}>
            <TodolistInfo className={'mx-5'}>
                <h4>
                    <b>Todolist 이름 : </b> {todoTitle}
                    <button
                        type="button"
                        className="btn btn-danger mx-3"
                        onClick={clickDeleteTodolist}>
                        투두리스트 삭제
                    </button>
                </h4>
                <h4>
                    <b>생성한 사람 : </b> {todoCreator}
                </h4>
            </TodolistInfo>
            <Grid>
                <TodoSection
                    ...
                    {todo
                        ? todo.map((item, key) => (
                              <div key={key} id={item.tddId} draggable={true} onDragStart={addRefs}>
